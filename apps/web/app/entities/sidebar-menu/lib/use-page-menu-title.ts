...
import type { RenderNavNode, SidebarMenuRoute } from "~/entities/sidebar-menu/model/sidebar-menu.types"

// 사이드바 트리(primaryNav/secondaryNav)에서 특정 route가 가리키는 메뉴의 현재 라벨을 찾는다.
function findMenuLabel(nodes: RenderNavNode[], targetRoute: SidebarMenuRoute): string | undefined {
...
export function usePageMenuTitle(route: SidebarMenuRoute, fallback: string): string {
