...
import type { DiffChunk, DocumentRevision, DocumentSeries, SeriesWithLatestRevision } from "~/entities/document/model/document.types"
...
function toRevision(row: RevisionRow): DocumentRevision {
  return {
    id: row.id,
...
export async function createRevision(input: CreateRevisionInput): Promise<DocumentRevision> {
  ...
  return toRevision(row as RevisionRow)
}
