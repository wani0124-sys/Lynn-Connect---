...
src/
  domains/                 # 비즈니스 도메인 단위 기능 묶음
    ...
    mail-archive/           # 메일 저장, 조회, 검색 도메인
      mail.route.ts         # GET /mails (목록), POST /mails (새 메일 저장)
      mail.service.ts       # 메일 요약, 정제, 권한 검사
      mail.repository.ts    # Supabase emails 테이블 접근, RLS 적용
      mail.schema.ts
      mail.middleware.ts    # 본사/현장 권한 가드
      # 2026-07-09: apps/worker가 아직 없어 이 구조 대신 apps/web/app/features/mail-archive
      # (React Router server action, service role key로 Supabase 직접 호출)로 1차 구현했다.
      # 다른 도메인이 별도 API가 필요해지는 시점에 이 표준 구조로 분리하는 것을 검토한다.

    standard-docs/          # 회사 표준/기준 문서 관리 도메인
      standard.route.ts     # GET /standards (목록), POST upload, DELETE
      standard.service.ts   # 버전 관리, 메타데이터 처리
      standard.repository.ts
      standard.schema.ts

    inspection-report/      # 현장 점검 보고서 도메인
      report.route.ts       # GET, POST (작성), PUT (수정)
      report.service.ts     # 상태(작성중/제출/피드백/완료)
