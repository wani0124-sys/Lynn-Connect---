...
4. **문서 리비전 관리(`/documents`)**: "현장 주요 업무 체크리스트"처럼 반복 개정되는 회사 표준 PDF 문서를 시리즈·리비전(Rev.0, Rev.1, ...) 단위로 관리하고, 새 리비전 업로드 시 PDF 텍스트를 추출해 직전 리비전과 줄 단위 diff를 자동 계산해 보여준다(2026-07-13). 문서 시리즈 생성·리비전 업로드는 본사 전용, 읽기는 전체 공개.
